...
type Capture = {
  ...
  pokemon: string;
  nickname: string;
  route: string | null; // texto (para mostrar)
  route_id: number | null; // ✅ clave
  game_id: number | null; // ✅ clave
...
export default function MiPanelPage() {
  ...
  const [routeId, setRouteId] = useState<number | "">(""); // ✅ select
  ...
  // nickname para guardar en team_slots
  ...
  // ✅ rutas disponibles según juego activo
  ...
  // si cambia el juego activo, resetea el selector de ruta
  useEffect(() => {
    setRouteId("");
  }, [activeGameId]);
  ...
  async function loadRunData() {
    // runs (singleton id=1)
    const run = await supabase.from("runs").select("active_game_id").eq("id", 1).single();
    if (!run.error) setActiveGameId((run.data?.active_game_id as number | null) ?? null);

    const g = await supabase.from("games").select("id, name").order("name");
    if (!g.error) setGames((g.data ?? []) as any);

    const r = await supabase.from("routes").select("id, game_id, name, name_es")
  ...
  async function loadAll() {
    ...
    // siempre cargamos run data (para que el selector de rutas funcione aunque no estés logueado)
    await loadRunData();
    ...
    const p = await supabase.from("profiles").select("id, display_name, role").eq("id", uid).single();
    ...
    const team = await supabase.from("team_slots").select("*").eq("user_id", uid).order("slot");
    ...
    const pr = await supabase.from("progress").select("user_id, badges, updated_at").eq("user_id", uid).maybeSingle();
    if (!pr.error) {
  ...
  async function addCapture() {
    ...
    const ins = await supabase
      ...
      .insert({
        ...
        route_id: routeId,
        // ✅ opcional: guardamos también el texto (para mostrar fácil)
        route: rName,
      })
      .select("*")
      .single();
  ...
  async function removeFromTeamIfNotAlive(pokemonName: string) {
    ...
    setBusy(true);
    const del = await supabase.from("team_slots").delete().eq("user_id", userId).eq("pokemon", p);
    setBusy(false);
  ...
  async function updateCaptureStatus(id: number, newStatus: CaptureStatus) {
    ...
    setBusy(true);
    const up = await supabase.from("captures").update({ status: newStatus }).eq("id", id);
    setBusy(false);
  ...
  async function deleteCapture(id: number) {
    ...
    setBusy(true);
    const del = await supabase.from("captures").delete().eq("id", id);
    setBusy(false);
  ...
  async function setSlot(slot: number, pokemonName: string) {
    ...
    for (const [slotStr, val] of Object.entries(teamInput)) {
      ...
      if (s === slot) continue;
      if ((val ?? "").trim().toLowerCase() === key) {
        return setMsg("Ese Pokémon ya está en tu equipo. Solo puede estar una vez.");
      }
    }
  ...
  async function clearSlot(slot: number) {
    ...
    setBusy(true);
    const del = await supabase.from("team_slots").delete().eq("user_id", userId).eq("slot", slot);
    setBusy(false);
    ...
    <div style={{ width: "100%", maxWidth: 1100, margin: "0 auto", padding: "0 16px" }}>
      ...
      <div style={{ marginTop: 18, ...cardStyle() }}>
        ...
        <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))" }}>
          ...
          {/* ✅ Ruta como select (depende del juego activo) */}
          <select
            required
            value={routeId}
          ...
          >
            <option value="">
              {activeGameId ? "Ruta (obligatorio)" : "Sin juego activo (admin)"}
            </option>
